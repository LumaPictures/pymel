...
# we use this since Python 2.6+ derives this from collections.MutableSequence which makes it harder to use
class OldUserList:
    ...
    def __init__(self, initlist=None):
        ...
        if initlist is not None:
            ...
            if type(initlist) == type(self.data):
                self.data[:] = initlist
            elif isinstance(initlist, OldUserList):
    ...
    def __cast(self, other):
        if isinstance(other, OldUserList): return other.data
        else: return other
    ...
    def __setslice__(self, i, j, other):
        i = max(i, 0); j = max(j, 0)
        if isinstance(other, OldUserList):
...
def optional_cm(cm, use_cm=True): # See naming note below
    """
    Optional Context Manager - Useful when a 'with' statement is necessary but no valid context-manager are available.
