...
class path(_base):
    ...
    def listdir(self, pattern=None, realpath=False):
        ...
        items whose names match the given pattern. Pattern may
        be either a shell 'glob' string, or a regular expression
        pattern object (generated by re.compile). Regexp's are
        tested using .match(), not .search()
        """
        ...
        results = [self / child for child in names]
        if realpath:
            results = [p.realpath() for p in results]
        return results
    ...
    def dirs(self, pattern=None, realpath=False):
        ...
        """
        return [p for p in self.listdir(pattern, realpath=realpath) if p.isdir()]
    ...
    def files(self, pattern=None, realpath=False):
        ...
        d.files('*.pyc').
        """
        return [p for p in self.listdir(pattern, realpath=realpath) if p.isfile()]
