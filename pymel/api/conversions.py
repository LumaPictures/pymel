...
#            _logger.warn( "failed to find docs for current version: %s", name )
    ...
    def __init__(self, rebuildAllButClassInfo=False):
        """
...
_start = time.time()
apiCache = ApiCache(rebuildAllButClassInfo=False)
...
def addMayaType(mayaType, apiType=None):
    ...
    if apiType is not 'kInvalid' :
        ...
        defType = apiCache.reservedMayaTypes.has_key(mayaType)
        
        apiCache.mayaTypesToApiTypes[mayaType] = apiType
        if not apiCache.apiTypesToMayaTypes.has_key(apiType) :
            apiCache.apiTypesToMayaTypes[apiType] = { mayaType : defType }
        else :
            apiCache.apiTypesToMayaTypes[apiType][mayaType] = defType
        ...
        apiCache.mayaTypesToApiEnums[mayaType] = apiEnum
        if not apiCache.apiEnumsToMayaTypes.has_key(apiEnum) :
            apiCache.apiEnumsToMayaTypes[apiEnum] = { mayaType : None }
        else:
            apiCache.apiEnumsToMayaTypes[apiEnum][mayaType] = None 
...
def removeMayaType(mayaType):
    ...
    """
    try:
        apiEnum = apiCache.mayaTypesToApiEnums.pop( mayaType )
    except KeyError: pass
    else:
        enums = apiCache.apiEnumsToMayaTypes[apiEnum]
    ...
    try:
        ...
        types = apiCache.apiTypesToMayaTypes[apiType]
        types.pop( mayaType, None )
        if not types:
            apiCache.apiTypesToMayaTypes.pop(apiType)
            apiCache.apiTypesToApiEnums.pop(apiType)
