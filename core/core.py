"""
The core module contains all of the functions which do not fall under the category of `pymel.core.ui`, `pymel.core.node`, or `ctx`.
"""
...
import pymel.util, pymel.util.factories, pymel.core.node, pymel.core.ui, pymel.core.io
...
def pythonToMel(arg):
    ...
    elif pymel.util.isIterable(arg):
...
class Mel(object):
    ...
    pymel:
        ...
        >>> mel.myScript( pymel.core.node.type(), pymel.core.node.color.get() )
...
class WorkspaceEntryDict(object):
    ...
    def items(self):    
        entries = pymel.util.listForNone( cmds.workspace( **{'q' : 1, self.entryType : 1 } ) )
        res = []
...
class Workspace(pymel.util.Singleton):
    """
    ...
    All paths are returned as an pymel.core.io.Path class, which makes it easy to alter or join them on the fly.    
    ...
    @classmethod
    def getPath(self):
        return pymel.core.io.Path(cmds.workspace( q=1, fn=1 ))
    ...
    @classmethod
    def getcwd(self):
        return pymel.core.io.Path(cmds.workspace( q=1, dir=1 ))
    @classmethod
    def mkdir(self, newdir):
        return cmds.workspace( cr=newdir )

    name = property( lambda x: cmds.workspace( q=1, act=1 ) )        
    path = property( lambda x: pymel.core.io.Path(cmds.workspace( q=1, fn=1 ) ) )
...
class FileInfo( pymel.util.Singleton ):
    """
...
class OptionVarDict(pymel.util.Singleton):
    """ 
...
class Env(pymel.util.Singleton):
...
class Scene(pymel.util.Singleton):
    def __getattr__(self, obj):
        return pymel.core.node.PyNode( obj )
...
NOTE: this command also reorders the argument order to be more intuitive, with the object first
    """
    if len(args) == 1 and pymel.util.isIterable(args[0]):
        args = tuple(args[0])
    args = args + (obj,)
...
Modifications:
    ...
    """
    source = pymel.core.node.Attribute(source)
    if destination:
...
Modifications:
    ...
    # perhaps it errored because it's a multi attribute
        ...
        attr = pymel.core.node.Attribute(attr)
        if attr.isMulti():
...
Modifications:
    ...
    def makePairs(l):
        ...
        for i in range(0, len(l),2):
            res.append( ( pymel.core.node.PyNode(l[i]), pymel.core.node.PyNode(l[i+1]) )  )
        return res
    ...
    if kwargs.get('connections', kwargs.get('c', False) ) :    
        ...
        return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.listConnections( *args,  **kwargs )) )
...
Modifications:
    ...
    if 'type' in kwargs:
        typ = kwargs.pop('type')
        return filter( lambda x: cmds.nodeType(x) == typ, map( pymel.core.node.PyNode, cmds.listHistory( *args,  **kwargs ) )  )
    return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.listHistory( *args,  **kwargs ) ) )
...
Modifications:
    ...
    return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.listRelatives(*args, **kwargs)))
...
Modifications:
    ...
    if kwargs.pop('editable', False):
        allNodes = pymel.util.listForNone(cmds.ls(*args, **kwargs))
        kwargs['readOnly'] = True
        kwargs.pop('ro',True)
        roNodes = pymel.util.listForNone(cmds.ls(*args, **kwargs))
        
        # faster way?
        return map( pymel.core.node.PyNode, filter( lambda x: x not in roNodes, allNodes ) )
    ...
    #return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.ls(*args, **kwargs)))
    ...
    if kwargs.get( 'showType', kwargs.get('st', False) ):
        tmp = pymel.util.listForNone(cmds.ls(*args, **kwargs))
        res = []
        for i in range(0,len(tmp),2):
            # res.append( pymel.core.node.PyNode( tmp[i], tmp[i+1] ) )
            res.append( pymel.core.node.PyNode( tmp[i] ) )
            res.append( tmp[i+1] )
        return res    
    ...
    if kwargs.get( 'readOnly', kwargs.get('ro', False) ):
        ...
        for pymel.core.node in ro:
            try:    
                ...
                all.pop(idx)
                typ = all.pop(idx+1)
                res.append( pymel.core.node.PyNode( pymel.core.node, typ ) ) 
                if showType:
                    res.append( typ )
...
Modifications:
    ...
    - returns wrapped classes
    """
    return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.lsThroughFilter(*args, **kwargs)))

    #for i in cmds.ls(*args, **kwargs):
    #    yield pymel.core.node.PyNode(i)
...
Modifications:
    ...
    return map( pymel.core.node.PyNode, res, ['transform']*len(res) )
...
Modifications:
    ...
    - returns wrapped classes
    """
    return map( pymel.core.node.Attribute, pymel.util.listForNone(cmds.listAnimatable( *args, **kwargs ) ) )
...
Modifications
    ...
    if not args and not cmds.ls(sl=1):
        kwargs['empty'] = True
    return pymel.core.node.Transform( cmds.group(*args, **kwargs) )
    #except RuntimeError, msg:
    #    print msg
    #    if msg == 'Not enough objects or values.':
    #        kwargs['empty'] = True
    #        return pymel.core.node.Transform( cmds.group(**kwargs) )
...
Modifications:
    - returns wrapped classes
    """
    return map(pymel.core.node.PyNode, cmds.duplicate( *args, **kwargs ) )
...
Modifications:
    - returns wrapped classes
    """
    return map(pymel.core.node.PyNode, cmds.instance( *args, **kwargs ) )    
...
Modifications:
    ...
    return map(pymel.core.node.PyNode, pymel.util.listForNone(cmds.attributeInfo(*args, **kwargs)))
'''
...
Modifications:
    ...
    if isinstance( newname, pymel.core.node.PyNode ):
        newname = newname.name()
    if isinstance (obj, pymel.core.node.PyNode) :
        obj = obj.name()
        
    return pymel.core.node.PyNode( cmds.rename( obj, newname, **kwargs ) )
    
def createNode( *args, **kwargs):
    return pymel.core.node.PyNode( cmds.createNode( *args, **kwargs ) )
...
Modifications
    ...
    for flag, value in kwargs.items():    
        if flag in setSetFlags:
            ...
            if pymel.util.isIterable(value):
                args = tuple(value)
    ...
    if kwargs.get( 'query', kwargs.get('q',False) ):
        ...
        if size == 1 or (size==2 and kwargs.get( 'nodesOnly', kwargs.get('no',False) )  ) :
            return map( pymel.core.node.PyNode, pymel.util.listForNone(cmds.sets( *args, **kwargs )) )
...
Modifications:
    ...
    """
    if len(args) ==1 and pymel.util.isIterable(args[0]) and not args[0]:
        return
'''    
...
def _createFunctions():
    for funcName in pymel.util.factories.moduleCmds['core']:
        func = pymel.util.factories.functionFactory( funcName, None, _thisModule )
        if func:
            func.__module__ = __name__
            setattr( _thisModule, funcName, func )
    for funcName in pymel.util.factories.getUncachedCmds():
